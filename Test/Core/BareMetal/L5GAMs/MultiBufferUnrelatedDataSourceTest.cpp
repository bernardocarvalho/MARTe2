...
class MultiBufferUnrelatedDataSourceTestInputBroker: public MemoryMapUnrelatedInputBroker {
public:
    CLASS_REGISTER_DECLARATION()

MultiBufferUnrelatedDataSourceTestInputBroker    () {

    }

    virtual ~MultiBufferUnrelatedDataSourceTestInputBroker() {

    }
};

CLASS_REGISTER(MultiBufferUnrelatedDataSourceTestInputBroker, "1.0")

class MultiBufferUnrelatedDataSourceTestOutputBroker: public MemoryMapUnrelatedOutputBroker {
public:
    CLASS_REGISTER_DECLARATION()

MultiBufferUnrelatedDataSourceTestOutputBroker    () {

    }

    virtual ~MultiBufferUnrelatedDataSourceTestOutputBroker() {

    }
};

CLASS_REGISTER(MultiBufferUnrelatedDataSourceTestOutputBroker, "1.0")
...
static const char8 * const config1 = ""
        ...
        "                   Type = uint32"
        "               }"
        "               Signal5$MultiBufferUnrelatedDataSourceTestInputBroker = {"
        "                   DataSource = Drv1"
        "                   Type = uint32"
        "               }"
        "            }"
        ...
        "                   Trigger = 1"
        "               }"
        "               Signal1$MultiBufferUnrelatedDataSourceTestOutputBroker = {"
        "                   DataSource = Drv1"
        "                   Type = uint32"
        "               }"
        "            }"
...
bool MultiBufferUnrelatedDataSourceTest::TestGetInputBrokers_SuggestedBroker() {
...
bool MultiBufferUnrelatedDataSourceTest::TestGetInputBrokers_False_InvalidSuggestedBroker() {
    bool ret = InitialiseMemoryMapInputBrokerEnviroment(config);
    ...
    if (ret) {
        ...
        if (ret) {
            ret = data->MoveToChild(0);
        }
        if (ret) {
            if (data->MoveRelative("InputSignals")) {
                data->MoveToChild(0);
                StreamString brokerName = dataSource->GetBrokerName(*data, InputSignals);
                ret = data->Delete("Broker");
                if (ret) {
                    StreamString invalidBrokerName = "InvalidBroker";
                    data->Write("Broker", invalidBrokerName);
                }
                if (ret) {
                    StreamString gamPath = "Application1.Functions.GAMA";
                    ReferenceT<MultiBufferUnrelatedDataSourceTestGAM1> gam = ObjectRegistryDatabase::Instance()->Find(gamPath.Buffer());
                    ReferenceContainer inputBrokers;
...
bool MultiBufferUnrelatedDataSourceTest::TestGetOutputBrokers_SuggestedBroker() {
...
bool MultiBufferUnrelatedDataSourceTest::TestGetOutputBrokers_False_InvalidSuggestedBroker() {
    bool ret = InitialiseMemoryMapInputBrokerEnviroment(config);
    ...
    if (ret) {
        ...
        if (ret) {
            if (data->MoveRelative("OutputSignals")) {
                ...
                if (ret) {
                    StreamString gamPath = "Application1.Functions.GAMB";
                    ReferenceT<MultiBufferUnrelatedDataSourceTestGAM1> gam = ObjectRegistryDatabase::Instance()->Find(gamPath.Buffer());
                    ReferenceContainer outputBrokers;
