 ...
 * @details For each element to be added to the list, an integer key is computed using the D. J. Bernstein hash function (see HashFunction).
...
public:
     ...
     */
    uint32 Insert(const char8 * const id,
                  const T value);
     ...
     * @return true if the element is in the list, false otherwise.
     */
    bool Search(const char8 * const id,
                uint32 &index);
...
private:
    ...
    bool BinarySearch(const uint32 key,
                      uint32 &index);
};
...
template<typename T, typename HashObject>
uint32 BinaryTree<T, HashObject>::Insert(const char8 * const id,
                                         const T value) {
...
template<typename T, typename HashObject>
bool BinaryTree<T, HashObject>::Search(const char8 * const id,
                                       uint32 &index) {
