...
bool ConfigurationDatabase::Read(const char8 * const name,
    ...
    if ((isRegisteredObject) || (isStructuredDataI)) {
        ...
        if (ok) {
            ...
            ReferenceT < AnyObject > objToRead = currentNode->Find(name, false, true);
...
bool ConfigurationDatabase::MoveToAncestor(const uint32 generations) {
    ...
    if (ok) {
#if 1
        ReferenceT < ReferenceContainer > oldCurrent = currentNode;
...
bool ConfigurationDatabase::CreateNodes(const char8 * const path) {
    ...
    while ((pathStr.GetToken(token, ".", c)) && (ok)) {
        ...
        if (ok) {
            ...
            Reference foundReference = currentNode->Find(token.Buffer(), false, true);
            if (foundReference.IsValid()) {
                ...
                if (ok) {
                    container->SetFather(currentNode);
                    currentNode = container;
                    created = true;
                }
            }
...
bool ConfigurationDatabase::Delete(const char8 * const name) {
    Reference foundReference = currentNode->Find(name, false, true);
