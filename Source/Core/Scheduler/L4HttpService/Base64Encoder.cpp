...
public:
    ...
    static const char8 *base64Alphabet = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=";
...
Base64Codec::Base64Codec() {
    ...
    for (uint32 i = 0u; i < 64u; i++) {
        decodingTable[base64Alphabet[i]] = i;
    }
    decodingTable['='] = 0xFEu;
...
bool Base64Encoder::Encode(StreamString &input,
                           StreamString &output) {
...
bool Base64Encoder::Decode(StreamString &input,
                           StreamString &output) {

    uint32 size = input.Size();
    const char8 *in = input.Buffer();
    output = "";
    uint32 word = 0;
    uint32 code;
    char8 key;
