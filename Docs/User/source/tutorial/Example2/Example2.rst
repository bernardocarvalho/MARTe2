...
In this case we will read from a LinuxTimer, execute a WaveformSin and print the value of the sine to a LoggerDataSource at a fixed frequency of 10 Hz. As in the previous one, no additional components are necessary in this example so we just need to update the configuration file. Lets see how it looks like:
